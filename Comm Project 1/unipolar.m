...
[W1 L1] = size(xx);
for i = 1:L1
    stat_mean_mat = [stat_mean_mat sum(xx(:,i))/W1];
end
stat_mean = sum(stat_mean_mat)/L1;
%--------------------------------------------------------------
%time mean
%---------------------------------------------------------------
time_mean = sum(xx(1,:))/L1;
%---------------------------------------------------------------
...
%rm=[];
% for shift = 1:L1
%     rand_shift = randi([0,1],1,shift);
%     rand_shift=((2*rand_shift)-1)*A;
%     R(shift+1,1:shift)=rand_shift;
%     R(shift+1,shift+1:end)=R(1,1:L1-shift);
% end
% [N V]=size(R);
% for i=0:N-1
%     rm=[rm sum(R(1,:).*R(i+1,:))./V];
% end
% 
% rho = [fliplr(rm) rm(2:end)];
% figure(2)
% tau2=-N:1:N-2
% plot(tau2,rho)
